...
def white_background_highlight(
    ax: matplotlib.axes.Axes, x: NDArray, y: NDArray
) -> None:
...
def main() -> None:
    ...
    predictions = np.fromfile(
        "./outputs/predictions_DS1_2_Cp_Cp_distribution.npy",
        dtype=np.float32,
    ).reshape([-1, 300])
    ...
    # selected_indices = [30, 20, 10]  # Fig 3
    # selected_indices = [200, 800, 1100]  # Fig 4
    # selected_indices = [200, 500, 600]  # Fig 5
    # selected_indices = [0, 650, 1300]  # Fig 6
    # selected_indices = [300, 650, 1300]  # Fig 7

    selected_indices = [300, 600, 729]  # Reviwer - instantaneous snapshots
    selected_indices = [30, 20, 40]
